...
.env/.up-to-date: base_requirements.txt flask_app/pip_requirements.txt Makefile
	...
	.env/bin/pip install pytest flask-loopback requests mailboxer-python
...
deploy_localhost_travis: deploy_localhost
	sleep 20 # travis uses slow nodes and tends to take time to bring the uwsgi app online
...
travis_test: travis_system_install deploy_localhost_travis
	.env/bin/py.test tests/ --www-port=80 --no-setup-db
