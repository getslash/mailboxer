...
def test_incoming_message_no_mailbox_match(flask_app):
...
def test_send_receive_email(webapp, recipient, email):
    ...
    received.pop("id")
    assert received == {'message': email.message, 'fromaddr': email.fromaddr, 'sent_via_ssl': False, 'read': False}
...
def test_mailbox_activity_gets_updated(webapp, inactive_recipient, unsent_email, flask_app):
    with flask_app.app_context():
...
def test_vacuum(webapp, inactive_recipient, recipient, flask_app):
    with flask_app.app_context():
